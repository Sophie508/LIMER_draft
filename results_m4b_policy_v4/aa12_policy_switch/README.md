...
| AA12_POLICY Loss fault, switch/stay policy decides (demonstration) | 3 | 1.001 | 0/3 |

Observed gate failures:

- `aa12_policy_rep01`: run status must be complete | correctness must pass (selected retention 1.001).
- `aa12_policy_rep02`: run status must be complete | correctness must pass | fault interface must remain up | detector interface must remain up | fault injector and detector interfaces must be distinct | detector must not read the injector qdisc | C3 is missing fault_period_retention (selected retention n/a).
- `aa12_policy_rep03`: run status must be complete | correctness must pass | fault interface must remain up | detector interface must remain up | fault injector and detector interfaces must be distinct | detector must not read the injector qdisc | C3 is missing fault_period_retention (selected retention n/a).
